...
ERT_SUBNET_CMD="az network vnet subnet list -g ${AZURE_TERRAFORM_PREFIX} --vnet-name ${AZURE_TERRAFORM_PREFIX}-virtual-network --output json | jq '.[] | select(.name == \"ert\") | .id' | tr -d '\"'"
